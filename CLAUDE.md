...
0. [.claude/commands/new-task.md](.claude/commands/new-task.md) — slash command `/new-task <redmine-id>` (ID thuần, vd `/new-task 40539`; URL đầy đủ vẫn nhận). **Bước CHUẨN BỊ INPUT** (KHÔNG phải skill review/write): fetch Redmine **qua REST API** (`python scripts/redmine_fetch.py <id|url>`, KHÔNG dùng MCP) → tạo folder `tasks/<YYYY-MM-DD>_<id>_<slug>/` + auto-fill `01-bug-task.md` (từ Section "Tái hiện bug") + `03-dev-impact.md` (từ Section "Đánh giá ảnh hưởng") + `04-tc-list.md`. Nguồn TC cho file 04, theo thứ tự: (a) Section "Link TCs" + `Row: <start>-<end>` của Redmine → Google Sheet; (b) **nếu Redmine KHÔNG có Link TCs human → fallback fetch từ MCP LME TEST STUDIO** (`task_list(ticket_id=...)` → `testcase_list`), xem BƯỚC 6b. DỪNG sau khi xong — KHÔNG tự chain `/write-tc` hoặc `/review-tc`.
1. [.claude/commands/review-tc.md](.claude/commands/review-tc.md) — slash command `/review-tc <folder> [ticket_id|task:<n>]`. Quy trình review cho Test Leader (BƯỚC 0 lấy TC + BƯỚC 1 → 6). **Operating instruction** khi user invoke `/review-tc` hoặc nhận task review TC.
   ...
   - **BƯỚC 4b** rà TC trùng lặp nội dung (DUP-EXACT / SUBSET / INFLATE) → **§3 report**, đề nghị xóa/gộp; **không tự xóa**, không đề nghị xóa TC duy nhất cover 1 impact/quan điểm.
   - **BƯỚC 4c** rà **mâu thuẫn** → **§4 report** + 1 dòng §8: `CONF-TC` (2 TC cùng nội dung check nhưng expected loại trừ nhau) · `CONF-SPEC` (trái Business rule ở `spec-features/`) · `CONF-KHO` (trái TC ở `kho-tcs/`). Đọc **targeted bằng grep/sed**, KHÔNG nạp cả file kho/spec. **Không tự chọn bên**, nêu đủ 2 khả năng (TC sai / spec-kho cần update). Bắt buộc fill kể cả khi sạch.
   - **BƯỚC 4d** rà **TC thừa / ngoài phạm vi task** → **§6 report**: flag khi đủ CẢ 3 (không map `BUG`/`F*`/`D*`/`T*` · không nằm trong diff + rủi ro hồi quy Studio · không thuộc quan điểm nào trigger), cộng TC test layer không bị chạm code. Gate: **không flag TC duy nhất cover 1 impact/quan điểm**, không flag TC regression dẫn được từ Studio/kho. Chỉ đề nghị, **không tự xóa**.
   - **BƯỚC 3a** chốt **phạm vi tính năng TRƯỚC khi quét 80 quan điểm** — ⛔ KHÔNG lọc Trigger theo danh sách file trong diff (Trigger của `SYNC-APP-001` · `OUT-EXPORT-001` · `BULK-001` · `LIST-001` · `JOB-001` · `INTG-*` là thuộc tính của **màn hình**, không có trong diff). Bắt buộc mở bảng **"Coverage theo màn hình/chức năng"** của `kho-tcs/fa<xxx>-*.md` và quét **từng dòng**, không chỉ grep nhóm trùng tên. Task chạm bảng/endpoint/hàm **dùng chung** → trả lời đủ **4 hướng đọc**: web · app quản trị di động · job nền · export & tích hợp ngoài. Ghi bảng quét ra scratchpad kèm 1 dòng lý do cho mỗi mã bị loại.
   - **BƯỚC 5a** trước khi đề xuất TC bổ sung phải đọc TC cũ ở [kho-tcs/](kho-tcs/) của tính năng → bắt vùng regression, phát hiện conflict expected (→ `CONF-KHO` ở §4 + §8 Spec update needed), và không đẻ TC trùng với bộ TC ở BƯỚC 0.
   - **§7 TC đề xuất sinh từ 3 nguồn**: `G<x>` (GAP/RISK ở §1 — **bắt buộc** có TC, trừ 2 ngoại lệ dùng lại TC kho / GAP giả) · `Q<x>` (§2) · **`R<x>` regression theo đánh giá của AI** (căn cứ: `dev_impact.rủi ro hồi quy` Studio / `<ID kho>` / caller của hàm dùng chung — không có căn cứ thì không đẻ TC).
   ...
   - ⚡ **Report rút gọn — chỉ ghi phần THIẾU, 9 section** (cấu trúc mới 2026-09-21): §0 Nguồn TC (**đúng 2 dòng**) · §1 Coverage (**2 dòng trả lời ĐỦ / CHƯA ĐỦ** cho chiều `dev-impact` + chiều `diff code`, rồi bảng GAP/RISK; **bỏ dòng `orphan`**) · §2 Quan điểm test còn thiếu · §3 TC trùng lặp · §4 **Mâu thuẫn trong TCs** (`CONF-TC` / `CONF-SPEC` / `CONF-KHO`) · §5 Issues khác · §6 **TCs thừa / ngoài phạm vi task** · §7 TC đề xuất bổ sung (14 cột, **tiêu đề kèm số TC**: `## 7. TCs đề xuất bổ sung (20)`) · §8 Spec update needed. §4 và §6 **bắt buộc fill kể cả khi sạch**.
   - **3 issue ĐÃ BỎ khỏi report (2026-09-21)** — không flag, không ghi ở bất kỳ section nào: cột `Evidence thực tế` rỗng · danh sách mã quan điểm không có trong `framework/checklist-lme.md` (hệ quả đã nằm ở §2) · checkbox “Tester verify auto-fill chính xác” chưa tick (checkbox vẫn còn cho `/new-task` + `/write-tc`).
...
3b. [.claude/commands/sync-review-tc.md](.claude/commands/sync-review-tc.md) — slash command `/sync-review-tc <folder> [studio|sheet|ask]`. Push **§7 "TCs đề xuất bổ sung"** của `05-review-report.md` về **ĐÚNG nơi bộ TC gốc được lấy về** (đọc §0 report):
...
Bảng TC ở `04-tc-list.md` dùng **16 cột canonical**, bám đúng sheet ["7. Ví dụ test case"](https://docs.google.com/spreadsheets/d/1IijLnq0gLZDFxOMWOYxXafz1Wnzv3W0g/edit?gid=1251796928#gid=1251796928) của Bảng quan điểm test HỢP NHẤT ELME v1.0:
...
- **TC No.** = `TC-<mã quan điểm bỏ dấu gạch>-<nn>`, đánh lại từ `01` cho mỗi quan điểm. VD `MSG-002` → `TC-MSG002-01`; `DATA-COUNT-001` → `TC-DATACOUNT001-03`. **Không** dùng `TC001`.
  - ⚠️ **Ngoại lệ — TC fetch từ MCP LME TEST STUDIO**: `TC No.` giữ **ID hiển thị trên tool** (`temp_id` dạng `NEW-7` / `NEW-28`; không có thì `#<studio id>`), mã theo quan điểm đẩy xuống cột `Ghi chú`. Lý do: Studio chỉ hiển thị `NEW-xx`, Leader phải tra ngược được. Mọi tham chiếu TC trong `05-review-report.md` (§1 → §6) dùng ID này; mã `TC-<quan điểm>-<nn>` **chỉ** dành cho TC mới đề xuất ở §7 (chưa tồn tại trên tool).
...
- Khi đề xuất TC bổ sung (**§7 report**): dùng **12 cột format kho** (giống output `/collect-tcs`, xem [kho-tcs/README.md](kho-tcs/README.md)) **+ 2 cột map với test tool** = **14 cột**, **KHÔNG** dùng 16 cột canonical của file 04:
  `ID` · `Nhóm` · `Mã quan điểm` · `Màn hình/chức năng` · `Loại case` · **`Chạy`** · **`Phạm vi ENV`** · `Tên case` · `Tiền điều kiện` · `Các bước thực hiện` · `Dữ liệu nhập` · `Kết quả mong đợi` · `Kết quả thực thi` · `Ghi chú`
  - `Chạy` = `auto` / `manual` → Studio `exec_mode`, **mặc định `auto`** (theo quy tắc của Studio "ĐỪNG mặc định manual"). `manual` **chỉ** khi bắt buộc production · thiết bị thật (app admin mobile / app LINE thật / camera) · mail thật · mắt người phán đoán — ghi `manual vì <lý do>` ở `Ghi chú`. TC nhóm `UI` và bước "bạn bè thao tác trên LINE" vẫn là `auto` (runner dùng browser tự động + mô phỏng callback). Chi tiết: [templates/05-review-report.template.md](templates/05-review-report.template.md) §7.
